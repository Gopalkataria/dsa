...
int main()
{
    long long int n, m, i = 0 , j = 0 , p, q , x ;
    scanf("%lld %lld", &n, &m);
    long long int count = n - 1, Uconn[n + 1],     Usize[n + 1],   heads[n + 1], printarr[n + 1];
    for ( i = 0; i <= n; i++)    Uconn[i] = i *   (Usize[i] = 1 + (heads[i] = 0));
    for ( x = 0; x < m; x++)
    {
        scanf("%lld %lld", &p, &q);
    ...
    printf("%lld\n", count);
    for ( x = 1; x < n + 1; x++)  if(heads[x] == 0)     printarr[j++] = x;
    for ( x = 1; x < j; x++)      printf("%lld %lld\n", printarr[x - 1],  printarr[x]);
    return 0;
}